...
async function renderNewOwner(req, res) {
  ...
  let { //changed to let to allow for automated corrections
    owner_first_name,
    owner_last_name,
    owner_email,
    owner_phone,
  ...
  owner_first_name = owner_first_name[0].toUpperCase() + owner_first_name.slice(1).toLowerCase();
  console.log(owner_first_name);
  owner_last_name = owner_last_name[0].toUpperCase() + owner_last_name.slice(1).toLowerCase();
  console.log(owner_last_name);
  //format the phone number as (XXX)XXX-XXXX
  owner_phone = owner_phone.replace(/[^a-zA-Z0-9 ]/g, '');
  owner_phone = "(" + owner_phone.slice(0,3) + ")" + owner_phone.slice(3,6) + "-" + owner_phone.slice(6);
